...
set EXTERNAL_CONFIG=
set /P VAULT="Using Vault configuration: [y/N] "
if "x%VAULT%"=="xy" (
  set EXTERNAL_CONFIG="vault"
)
...
echo kind: ConfigMap
echo metadata:
echo   name: kaudit-integration-%NAME%
echo   namespace: %NAMESPACE%
echo   labels:
echo     app: kaudit
echo     app-name: %NAME%
...
echo kind: ConfigMap
echo metadata:
echo   name: kaudit-data-filter-%NAME%
echo   namespace: %NAMESPACE%
echo   labels:
echo     app: kaudit
echo     app-name: %NAME%
echo data:
echo   audit-data-filter: ^|
echo.
echo ---
echo.
echo apiVersion: v1
...
if not "x"=="x%EXTERNAL_CONFIG%" (
echo         vault.hashicorp.com/agent-inject: "true"  >> "%DEPLOYMENT_FILE%"
echo         vault.hashicorp.com/role: "kaudit-vault-role" >> "%DEPLOYMENT_FILE%"
echo         vault.hashicorp.com/agent-inject-secret-%NAME%: "secret/data/%NAME%/config" >> "%DEPLOYMENT_FILE%"
echo         vault.hashicorp.com/agent-inject-template-%NAME%: ^| >> "%DEPLOYMENT_FILE%"
echo           {{- with secret "secret/data/%NAME%/config" -}} >> "%DEPLOYMENT_FILE%"
echo           {{ range \$k, \$v := .Data.data }} >> "%DEPLOYMENT_FILE%"
echo           "{{ $k }}": "{{ $v }}" >> "%DEPLOYMENT_FILE%"
echo           {{ end }} >> "%DEPLOYMENT_FILE%"
echo           {{- end -}} >> "%DEPLOYMENT_FILE%"
)
...
echo       - name: policy-volume
echo         configMap:
echo           name: kaudit-policy-%NAME%
echo           items:
echo             - key: audit-policy
echo               path: audit-policy.yaml
echo       - name: integration-volume
echo         configMap:
echo           name: kaudit-integration-%NAME%
echo           items:
echo             - key: audit-integration
echo               path: audit-integration.yaml
echo       - name: data-filter-volume
echo         configMap:
echo           name: kaudit-data-filter-%NAME%
echo           items:
echo             - key: audit-data-filter
echo               path: audit-data-filter.yaml
...
echo         - name: policy-volume
echo           mountPath: /kaudit/policy
echo         - name: integration-volume
echo           mountPath: /kaudit/integration
echo         - name: data-filter-volume
echo           mountPath: /kaudit/data-filter
echo         - name: data-volume
echo           mountPath: /data
echo         ports:
...
echo.
echo         - name: JAVA_OPTS
echo           value: -Xmx4G -Djava.security.egd=file:/dev/urandom -Dclojure.spec.skip-macros=true -Dclojure.compiler.direct-linking=true
echo.
echo         - name: EXTERNAL_CONFIG_FILE
echo           value: %EXTERNAL_CONFIG%
echo.
